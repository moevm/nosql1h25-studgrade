...
async def create_teacher(
    ...
    async with await db.client.start_session() as session:
        async with session.start_transaction():
            user_result = await users_collection.insert_one(user_model.mongo_dump(), session=session)
            if user_result.inserted_id is None:
                raise UserAlreadyExistsError("User with this email already exists.")
            print(type(user_result.inserted_id))
            teacher_model = TeacherModel.from_input(teacher, user_result.inserted_id)
            result = await teachers_collection.insert_one(teacher_model.mongo_dump(), session=session)
